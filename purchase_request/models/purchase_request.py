...
class PurchaseRequest(models.Model):
    ...
    def unlink(self, child=False):
        for rec in self:
            # statement = self.env['account.bank.statement.line'].search([('move_id','=',rec.id)])
            if rec.name:
                raise UserError(_("No puede eliminar una solicitud de compra con un numero asignado, favor cancelar o dejar cancelado."))
        return super().unlink()
